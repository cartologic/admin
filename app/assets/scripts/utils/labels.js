module.exports = {
  sdsLabels: {
    'en': [
      'Pillar 1: Economic Development',
      'Pillar 2: Energy',
      'Pillar 3: Knowledge, Innovation and Scientific Research',
      'Pillar 4: Transparency & Efficient Government Institutions',
      'Pillar 5: Social Justice',
      'Pillar 6: Health',
      'Pillar 7: Education & Training',
      'Pillar 8: Culture',
      'Pillar 9: Environment',
      'Pillar 10: Urban Development'
    ],
    'ar': [
      'المحور الأول: التنمية الاقتصادية',
      'المحور الثاني: الطاقة',
      'المحور الثالث: المعرفة والابتكار والبحث العلمي',
      'المحور الرابع: شفافية وكفاءة المؤسسات الحكومية',
      'المحور الخامس: العدالة الاجتماعية',
      'المحور السادس: الصحة',
      'المحور السابع: التعليم والتدريب',
      'المحور الثامن: الثقافة',
      'المحور التاسع: البيئة',
      'المحور العاشر: التنمية العمرانية'
    ],
    'select': 'Select an SDS Pillars - يُرجى اختيار أحد أهداف استراتيجية التنمية المستدامة التى يتناولها المشروع'
  },

  sdgLabels: {
    'en': [
      'Goal 1: No Poverty',
      'Goal 2: Zero Hunger',
      'Goal 3: Good health and Well-being',
      'Goal 4: Quality Education',
      'Goal 5: Gender Equality',
      'Goal 6: Clean Water & Sanitation',
      'Goal 7: Affordable & Clean Energy',
      'Goal 8: Decent Work & Economic Growth',
      'Goal 9: Industry, Innovation & Infrastructure',
      'Goal 10: Reduced Inequalities',
      'Goal 11: Sustainable Cities & Communities',
      'Goal 12: Responsible Consumption & Production',
      'Goal 13: Climate Action',
      'Goal 14: Life Below Water',
      'Goal 15: Life On Land',
      'Goal 16: Peace, Justice and Strong Institutions',
      'Goal 17: Partnerships For The Goals'
    ],
    'ar': [
      'الهدف 1: القضاء على الفقر',
      'الهدف 2: القضاء على الجوع',
      'الهدف 3: الصحة الجيدة والرفاه',
      'الهدف 4: التعليم الجيد',
      'الهدف 5: المساواة بين الجنسين',
      'الهدف 6: المياه النظيفة والنظافة الصحية',
      'الهدف 7: طاقة نظيفة وباسعار معقولة',
      'الهدف 8: العمل اللائق ونمو الاقتصاد',
      'الهدف 9: الصناعة والإبتكار والهياكل الأساسية',
      'الهدف 10: الحدّ من أوجه عدم المساواة',
      'الهدف 11: مدن ومجتمعات محلية مستدامة',
      'الهدف 12: الاستهلاك والإنتاج المسؤولان',
      'الهدف 13: العمل المناخي',
      'الهدف 14: الحياة تحت الماء',
      'الهدف 15: الحياة في البرّ',
      'الهدف 16: السلام والعدل والمؤسسات القوية',
      'الهدف 17: عقد الشاراكات لتحقيق الأهداف'
    ],
    'select': 'Select an SDGs - يُرجى اختيار أحد أهداف التنمية المُستدامة التى يتناولها المشروع'
  },
  otherDevLabels: {
    'en': [
      'Education & Training',
      'Health',
      'Economy',
      'Agriculture',
      'Domestic Policy',
      'Energy',
      'Environment'
    ],
    'ar': [
      'التعليم والتدريب',
      'الصحة',
      'الإقتصاد',
      'الزراعة',
      'السياسة المحلية',
      'الطاقة',
      'البيئة'
    ],
    'select': 'Select Other Development Indicator'
  },
  fundLabelsInter: {
    'en': [
      'Loan',
      'Grant',
      'Local Contribution'
    ],
    'ar': [
      'قرض',
      'منحة',
      'مساهمة محلية'
    ]
  },
  fundLabelsNational: {
    'en': [
      'Government Budget Allocation',
      'In-kind',
      'Loan',
      'Private Sector',
      'Local Non-Profit Organization',
      'Grant'
    ],
    'ar': [
      'تخصيص من الميزانية الحكومية',
      'عينيًا',
      'قرض',
      'قطاع خاص',
      'جهة محلية غير هادفة للربح',
      'منحة'
    ]
  },
  donors: {
    'en': [
      'Academy of Scientific Research',
      'African Development Bank',
      'Agricultural Research and Development Fund',
      'Care international',
      'Central Bank of Egypt',
      'Coptic Evangelical Organization for Social Services',
      'Danish International Development Agency',
      'Department of Foreign Affairs, Trade and Development, Canada',
      'Egyptian Micro, Small and Medium Enterprises Development Agency',
      'European Bank for Reconstruction and Development',
      'European Investment Bank',
      'European Union',
      'European Union - Joint Rural Development Program',
      'Federal Ministry of Economic Cooperation and Development (BMZ)',
      'Food and Agriculture Organization of the United Nations',
      'French Development Agency (AfD)',
      'German Corporation for International Cooperation (GIZ)',
      'Government of Egypt',
      'Governmental Agriculture Associations',
      'HEIA Association',
      'International Fund for Agricultural Development',
      'International Labor Organization',
      'Islamic Development Bank',
      'Italian Agency for Development Cooperation',
      'Italian-Egyptian Debt Swap Programme',
      'Japan International Cooperation Agency',
      'Japan Social Development Fund',
      'Kreditanstalt für Wiederaufbau (KfW)',
      'Local Development Fund',
      'Ministry of Social Solidarity',
      'Project Beneficiaries',
      'Sawiris Foundation for Social Development',
      'Science and Technology Development Fund',
      'Social Development Associations',
      'Social Fund for Development, Egypt',
      'Swiss Development Cooperation',
      'The Agricultural Development Program',
      'The Drosos Foundation',
      'The Egyptian Agricultural Bank',
      'United Nations Development Funds for Women',
      'United Nations Development Programme',
      'United Nations Industrial Development Organization',
      'United States Agency for International Development',
      'Vodafone',
      'World Bank',
      'World Food Program',
      'WorldFish',
      'Adaptation Fund',
      'Global Environment Facility (GEF)',
      'Green Climate Fund',
      'OPEC Fund for International Development',
      'Private Sector',
      'United Nations Development Fund for Women',
      'United Nations Trust Fund for Human Security'

    ],
    'ar': [
      'أكاديمية البحث العلمي',
      'البنك الإفريقي للتنمية',
      'صندوق البحوث و التنمية الزراعية',
      'هيئة الإغاثة كير',
      'البنك المركزي المصري',
      'الهيئة الإنجيلية القبطية',
      'الوكالة الدانماركية للتنمية الدولية',
      'وزارة الشؤون الخارجية والتجارة والتنمية, كندا',
      'جهاز تنمية المشروعات الصغيرة والمتوسطة و متناهية الصغر',
      'البنك الأوروبي لإعادة البناء والتنمية',
      'بنك الاستثمار الأوروبي',
      'الاتحاد الأوروبي',
      'برنامج الاتحاد الأوروبي المشترك للتنمية الريفية',
      'الوزارة الاتحادية للتعاون الاقتصادي والتنمية (BMZ)',
      'منظمة الأغذية والزراعة',
      'الوكالة الفرنسية للتنمية (AfD)',
      'المؤسسة الألمانية للتعاون الدولي (GIZ)',
      'الحكومة المصرية',
      'الجمعيات الزراعية الحكومية',
      'جمعية هيا',
      'الصندوق الدولي للتنمية الزراعية',
      'منظمة العمل الدولية',
      'البنك الإسلامي للتنمية',
      'الوكالة الايطالية للتنمية و التعاون / التعاون الايطالي',
      'برنامج مبادلة الديون المصري الإيطالي',
      'الوكالة اليابانية للتعاون الدولي',
      'صندوق التنمية الإجتماعية اليابانية',
      'بنك الائتمان لإعادة الإعمار (KfW)',
      'صندوق التنمية المحلي',
      'وزارة التضامن',
      'مستفيدين المشروع',
      'مؤسسة ساويرس للتنمية الاجتماعية',
      'صندوق دعم العلوم والتكنولوجيا',
      'جمعيات تنمية المجتمع',
      'الصندوق الاجتماعي للتنمية, مصر',
      'الوكالة السويسرية للتعاون الدولي',
      'برنامج التنمية الزراعية',
      'مؤسسة دروسس',
      'البنك الزراعي المصري',
      'صندوق الأمم المتحدة الإنمائي للمرأة',
      'برنامج الأمم المتحدة الإنمائي',
      'منظمة الأمم المتحدة للتنمية الصناعية',
      'الوكالة الامريكية للتنمية الدولية',
      'فودافون',
      'البنك الدولي',
      'برنامج الأغذية العالمي',
      'المركز الدولي للأسماك',
      'صندوق التكيف',
      'صندوق البيئة العالمي',
      'صندوق المناخ الأخضر',
      'صندوق الأوبك للتنمية الدولية',
      'قطاع خاص',
      'صندوق الأمم المتحدة الإنمائي للمرأة',
      'صندوق الأمم المتحدة الاستئماني للأمن البشري'
    ]
  },
  beneficiaryTypes: {
    en: [
      'Women',
      'Men',
      'Small Farmers (Less than 5 feddan)',
      'Large Farmers (5 feddan or more)',
      'Households',
      'Youth',
      'Female Head of Household',
      'Bedouins'
    ],
    ar: [
      'سيدات',
      'رجال',
      'مزارعين صغار (أقل من 5 فدان)',
      'مزارعين كبار (5 فدان أو أكثر)',
      'أسر معيشية',
      'شباب',
      'امراة معيلة',
      'بدو'
    ]
  }
};
